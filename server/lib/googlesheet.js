...
var xmlutil = Npm.require('xml2json');

// declare global namespace
GoogleSheet = {};

//public api
GoogleSheet.load = function (opts, callback) {
    if (!callback)
    ...
    auth(opts, Meteor.bindEnvironment(function (err, token) {
...
GoogleSheet.convertRangeToParams = function(range) {
    var r, cell, col;
    var paramsObj = {};
    ...
    // from cell
    if (r[0]) {
        col = r[0].match(/^[A-Za-z]*/)[0];
        paramsObj["min-col"] = columnToNumber(col);
        paramsObj["min-row"] = +r[0].replace(col, "");
    }

    // to cell
    if (r[1]) {
        col = r[1].match(/^[A-Za-z]*/)[0];
        paramsObj["max-col"] = columnToNumber(col);
        paramsObj["max-row"] = +r[1].replace(col, "");
    }
...
Spreadsheet.prototype.request = function (opts, callback) {
    ...
    //use pre-generated authentication headers
    ...
    Meteor.http.call(opts.method, opts.url, opts, function (err, response) {
        //show error
        if (err)
            return callback(err);
        ...
        var result;
        try {
            result = xmlutil.toJson(response.content, {object: true, sanitize: false, trim: false});
        } catch (e) {
...
Spreadsheet.prototype.receive = function (options, callback) {
    ...
    // pass parameters (ie: range) to the Google Sheets query
    strParams = "";
    if (options.params) {
...
Spreadsheet.prototype.receiveSync = Meteor.wrapAsync(Spreadsheet.prototype.receive);
